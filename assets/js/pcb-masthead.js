...
(() => {
  ...
  // --- Resistor SVG integration tuning (uses resistor-without-bg.svg artwork) ---
  // Cropped viewBox so the resistor fills its box (no giant square padding).
  // This crop is tuned for the attached resistor SVG (after removing the black background path).
  const RESISTOR_VIEWBOX = "16 112 480 288"; // width:height = 5:3
  const RESISTOR_ASPECT = 5 / 3;            // matches the cropped viewBox ratio
  const RESISTOR_LEAD_END_Y = 272 / 288;    // ≈0.944... where the legs end within the cropped viewBox
  const RESISTOR_SVG_CONTENT = window.PCB_RESISTOR_SVG_CONTENT || "";
  ...
  function buildSilkscreen(svgRoot, w, h, anchors = {}) {
    ...
    const yBot = anchors.yBot ?? snap(clamp(h * 0.82, h - 18, h - 10));

    // size resistor based on masthead height so it always fits
    const resH = snap(clamp(h * 0.28, 20, 34));
    const resW = snap(resH * RESISTOR_ASPECT);

    // position: keep it inside bounds + line up the “legs” with the bottom bus
    const resX = snap(clamp(w * 0.34, 10, w - resW - 10));
    const resY = snap(clamp(yBot - (resH * RESISTOR_LEAD_END_Y), 8, h - resH - 8));
  ...
  function rebuild() {
    ...
    try {
      ...
      // compute buses first so silkscreen resistor can align to them
      ...
      // silkscreen (now knows where the bottom bus is)
      ...
      // use x0/x1/yTop/yBot you computed above (DON'T redeclare them)
