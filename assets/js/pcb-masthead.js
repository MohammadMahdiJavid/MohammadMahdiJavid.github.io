...
(() => {
  ...
  // --- Resistor: simple SMD footprint that matches the PCB theme ---
  const RESISTOR_VIEWBOX = "0 0 120 40";

  const RESISTOR_SVG_CONTENT = `
    <g class="pcb-r">
      <!-- faint copper stubs so it visually "connects" into surrounding routing -->
      <path class="pcb-r__trace" d="M 0 20 L 6 20" />
      <path class="pcb-r__trace" d="M 114 20 L 120 20" />

      <!-- ENIG pads -->
      <rect class="pcb-pad pcb-r__pad" x="6"  y="13.5" width="20" height="13" rx="3.2" ry="3.2" />
      <rect class="pcb-pad pcb-r__pad" x="94" y="13.5" width="20" height="13" rx="3.2" ry="3.2" />

      <!-- body (component) -->
      <rect class="pcb-r__body" x="36" y="11" width="48" height="18" rx="5.5" ry="5.5" />

      <!-- silkscreen outline -->
      <rect class="pcb-r__silk" x="34" y="9" width="52" height="22" rx="7" ry="7" />

      <!-- courtyard / assembly outline (subtle dashed) -->
      <rect class="pcb-r__courtyard" x="2" y="6" width="116" height="28" rx="8" ry="8" />

      <!-- tiny "marking bands" for texture (kept subtle) -->
      <path class="pcb-r__band" d="M 55 12.8 L 55 27.2" />
      <path class="pcb-r__band" d="M 61 12.8 L 61 27.2" />
    </g>
  `;
  ...
  function buildResistorSilkscreen(parent, comp) {
    const resistor = svgEl("svg", {
      ...
      // footprint coordinates (not 512x512 icon space)
      viewBox: RESISTOR_VIEWBOX,
      preserveAspectRatio: "xMidYMid meet",

      // IMPORTANT: prevents pad drop-shadows from getting clipped inside the nested <svg>
      overflow: "visible",
  ...
  function buildSilkscreen(svgRoot, w, h) {
    ...
    const comps = [
      ...
      { x: w * 0.34, y: h * 0.70, ww: 36, hh: 12, label: "R2", type: "resistor", scale: 3.6 },
